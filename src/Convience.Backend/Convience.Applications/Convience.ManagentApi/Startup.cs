...
namespace Convience.ManagentApi
{
    public class Startup
    {
        ...
        {
            var dbConnectionString = Configuration.GetConnectionString("ConnDB");
            var jwtOption = Configuration.GetSection("JwtOption");

            services.AddControllers().AddControllersAsServices().AddNewtonsoftJson(
                //o=>o.SerializerSettings.ContractResolver=new DefaultContractResolver()//糶
                )
    ...
    public static class CustomExtension
    {
        public static IServiceCollection AddApplicationDbContext(this IServiceCollection services, string connectionString)
        {
            services.AddCustomDbContext<SystemIdentityDbContext, SystemUser, SystemRole, int, SystemUserClaim,
                ...
                (connectionString, DataBaseType.SqlServer);
        ...
        {
            services.AddHF(HangFireDataBaseType.SqlServer, connectionString);
            return services;
        }
