...
@Component({
  selector: 'app-supplier-batch-upload',
  templateUrl: './supplier-batch-upload.component.html',
  styleUrls: ['./supplier-batch-upload.component.less']
})
export class SupplierBatchUploadComponent implements OnInit {
  uploadForm: FormGroup = new FormGroup({});
  fileList: any[] = [];
  uploading: boolean = false;
  currentDirectory: string = 'vendor-batch-upload';
  constructor(private _messageService: NzMessageService,
    private _srmSupplierService: SrmSupplierService,
  ...
  download() {
    ...
    fileInfo.fileName = "SRM供應商批次上傳格式.xlsx"
  ...
  upload(directory, fileList) {
    ...
    this._srmSupplierService.BatchUpload(formData).subscribe(result => {
      console.log(result);
      this.fileList = [];
