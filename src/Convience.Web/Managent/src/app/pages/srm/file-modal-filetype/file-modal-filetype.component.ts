...
export class FileModalFiletypeComponent implements OnInit {
  ...
  beforeUpload = (singleFile: File, fileList: File[]): boolean => {
    if(this.inputFile.fileList==null)
    {
      this.inputFile.fileList = [];
    }
    if(this.inputFile.fileList.find(p=>p.name==singleFile.name))
    {
      return false;
    }
    else
    {
      this.inputFile.fileList = this.inputFile.fileList.concat(singleFile);
      //this.inputFile.fileList=fileList;
     //  const formData = new FormData();
      const emit =new FileEmit();
      emit.filtType=this.inputFile.filetype;
      emit.file=this.inputFile.fileList;
    ...
    this._modalService.confirm({
      ...
      nzOnOk: () => {
          this._srmFileService.delete(file.uid).subscribe(result => {
            ...
            this.inputFile.fileList=this.inputFile.fileList.filter(p=>p.uid!=file.uid);
          });
